...
- **Not done / open**: the stripes have never been *visually* confirmed — no browser tooling was available that session, so density is unverified. Dials are `tile = 8` and `lineWidth = 2.5` in `makePatternFill`. Also declined this round but still worth doing: the warm/cool palette split (steer needs to cool families, wants to warm ones in the swatch picker) and a "Needs vs Wants" split stat on the Spending page (`Needs $X (68%) / Wants $Y (32%)` + 6-month trend of the wants share) — the flag is already stored, so both are additive.
- **Next / in progress:** Nightly cron job for scheduled-sync still TODO; consider whether the Investments "Portfolio Value" stat should include retirement totals (currently holdings-only)
...
- **Next / in progress:** Nightly cron job for scheduled-sync still TODO; consider whether the Investments "Portfolio Value" stat should include retirement totals (currently holdings-only); the 2026-08-03 stripe density (`tile = 8`, `lineWidth = 2.5`) is still visually unconfirmed
